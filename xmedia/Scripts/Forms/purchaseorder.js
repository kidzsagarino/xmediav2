(function () {
    ...
    var formAssignedSizesLL = new LinkedList(null);
    var formPaperTypesLL = new LinkedList(null);
    var FormPrintColorsLL = new LinkedList(null);
    var formQuantitiesLL = new LinkedList(null);
    ...
    var eventsListeners = function () {
        ...
        document.querySelector('#js-companyLogoImg').addEventListener('change', function (e) {
            let reader = new FileReader();

            reader.onload = function () {
                //console.log(reader.result);
                let parentTR = e.target.closest('tr');
                parentTR.querySelector('.js-companyLogoPreview').removeAttribute('style');
                parentTR.querySelector('.js-companyLogoPreview img').src = reader.result;
            };
    ...
    var computePrice = function (paperType = null) {
        ...
        let paperSizeCost = formAssignedSizesLL.getByID(paperSizeSelect.value);
        ...
        let paperTypeCost = formPaperTypesLL.getByID(paperTypeSelect.value);

        let paperCostAtA3 = paperTypeCost != null ? paperTypeCost.PaperCostAtA3 : null;
        let laborCostAtA3 = paperTypeCost != null ? paperTypeCost.LaborCostAtA3 : null;

        let printCostBW = paperTypeCost != null ? paperTypeCost.PrintingCostAtA3BW : null;
        let printCostColored = paperTypeCost != null ? paperTypeCost.PrintingCostAtA3Colored : null;
        ...
        let quantity = formQuantitiesLL.getByID(paperQuantitySelect.value);

        let quantityValue = quantity != null ? quantity.FormQuantity : null;
    ...
    fetch(AppGlobal.baseUrl + 'Forms/GetFormSettings/1')
        ...
        .then(data => {
            ...
            data.FormPaperTypes.forEach((item) => {
                formPaperTypesLL.push(item);
            });

            //FormAssignedSizes
            data.FormAssignedSizes.forEach((item) => {
                formAssignedSizesLL.push(item);
            });

            //FormPrintColors
            data.FormPrintColors.forEach((item) => {
                FormPrintColorsLL.push(item);
            });

            //FormQuantities
            data.FormQuantities.forEach((item) => {
                formQuantitiesLL.push(item);
            });

            //console.log(formPaperTypesLL);
            //console.log(formAssignedSizesLL);
            //console.log(FormPrintColorsLL);
            //console.log(formQuantitiesLL);
